...
namespace Mojio.Platform.SDK.CLI
{
    ...
    public sealed class Parser
    {
        ...
        public Parser(Type argumentSpecification, ErrorReporter reporter)
        {
            ...
            this.argumentMap = new Dictionary<string, Argument>();
        ...
        private static ArgumentAttribute GetAttribute(PropertyInfo property)
        {
            ...
            if (attributes.LongCount() == 1)
                return (ArgumentAttribute)attributes.FirstOrDefault();
            ...
            foreach (var a in args)
            {
                if (a.Length > 0)
                {
                    switch (a[0])
                    {
                        ...
                        default:
                            newArgs[newArgs.Count - 1] = newArgs[newArgs.Count - 1] + " " + a;
                            break;
                    }
                }
            }
        ...
        private bool LexFileArguments(string fileName, out string[] arguments)
        {
            ...
            arguments = argArray.ToArray(); // (string[])argArray.ToArray(typeof(string));
            return hadError;
        }
        ...
        private class Argument
        {
            public Argument(ArgumentAttribute attribute, PropertyInfo property, ErrorReporter reporter)
            {
                ...
                if (IsCollection)
                {
                    this.collectionValues = new List<string>();
                }
            ...
            public bool Finish(object destination)
            {
                ...
                {
                    this.property.SetValue(destination, this.collectionValues.ToArray());
                    //.ToArray(this.elementType), null);
                }
            ...
            public bool SetValue(string value, object destination)
            {
                ...
                if (this.IsCollection)
                {
                    ...
                    {
                        this.collectionValues.Add(newValue.ToString());
                    }
                }
                else
                {
            ...
            private List<string> collectionValues;
